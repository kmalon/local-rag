...
Auth: OAuth2 Resource Server (JWT) via Keycloak realm `local-rag`, roles `rag_admin` (ingest) / `rag_user` (query) / `rag_mcp_user` (MCP). Two filter chains: `/mcp/**` also validates `aud` == `mcp.resource` (`http://localhost:8080/mcp`, injected by audience mapper on `rag-client`) and 401s with RFC 9728 `resource_metadata` hint; REST chain validates `iss` only, so foreign-client tokens still work. Metadata at `/.well-known/oauth-protected-resource[/mcp]` (public).
